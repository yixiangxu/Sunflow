...
import org.sunflow.core.shader.FogShader;
...
public class SphereLight implements LightSource, Shader {
    ...
    public void getSamples(ShadingState state) {
        ...
        if (l2 <= r2 && state.getShader().getClass() != FogShader.class)
        ...
        if (state.getNormal().dot(topX, topY, topZ) <= 0 && state.getShader().getClass() != FogShader.class)
        ...
        for (int i = 0; i < samples; i++) {
            ...
            if (cosNx <= 0 && state.getShader().getClass() != FogShader.class)
                continue;
